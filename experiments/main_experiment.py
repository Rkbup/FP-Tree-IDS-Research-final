...
Usage (project root):
    python experiments/main_experiment.py [--config CONFIG]
                                          [--output-dir DIR]
                                          [--raw-dir DATA_DIR]
                                          [--days Monday Tuesday ...]
                                          [--quiet]
...
print("Ctrl+C ignored for safe copy. Press Ctrl+H to stop.")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Limit worker processes/threads to N (defaults to CPU count).",
    )
    parser.add_argument(
        "--fast-mode",
        action="store_true",
        help="Faster run: limit to a small subset of transactions.",
    )
...
def evaluate_streaming_performance(
    ...
    """Evaluate algorithms over a transaction stream.

    - FP-tree variants use a sliding window and mine patterns periodically.
    - Baselines operate on one-hot vectors with periodic refits.
    """
    n = len(transactions)
    results: Dict[str, Dict[str, float]] = {}

    # Precompute one-hot features for baselines
    ...
    for i, txn in enumerate(transactions):
        for it in txn:
            X_full[i, idx_of[it]] = 1

    # Helper to process one transaction for Pool
    def _process(args):
        (name, alg, idx, txn, window_manager, cached_patterns,
         anomaly_threshold, refresh_interval) = args
        from src.algorithms.fp_tree import FPTree  # local import for Pool pickling
        from src.algorithms.variants.partial_rebuild import PartialRebuildFPTree
        from src.algorithms.variants.two_tree import TwoTreeFPTree
        from src.algorithms.variants.decay_hybrid import DecayHybridFPTree
        from src.algorithms.baselines.half_space_trees import HalfSpaceTrees
        ...
        if isinstance(alg, (FPTree, PartialRebuildFPTree, DecayHybridFPTree)):
            window_manager.insert(txn)
            if idx == 0 or ((idx + 1) % max(1, refresh_interval) == 0):
                cached_patterns.update(alg.mine_frequent_patterns())
            score = 1.0
            if cached_patterns:
            ...
            x = X_full[idx:idx+1]
            score = float(alg.score_samples(x)[0])
            pred = 1 if score >= anomaly_threshold else 0
            return score, pred
        else:
    ...
    for name, alg in algorithms.items():
        ...
        args_list = [
            (name, alg, idx, txn, window_manager, cached_patterns, anomaly_threshold, refresh_interval)
            for idx, txn in enumerate(transactions)
        ]

        if num_workers == 1:
            results_list = [_process(a) for a in args_list]
        else:
            with mp.Pool(processes=num_workers) as pool:
                results_list = pool.map(_process, args_list, chunksize=100)
        ...
        metrics['latency'] = (elapsed / max(1, n)) * 1000.0
...
def main() -> None:
    args = parse_args()
    # Optional thread limiting via env for BLAS/NumExpr
    if args.max_threads and args.max_threads > 0:
        for k in ("OMP_NUM_THREADS", "MKL_NUM_THREADS", "NUMEXPR_NUM_THREADS"):
            os.environ[k] = str(args.max_threads)
    workers = args.max_threads if (args.max_threads and args.max_threads > 0) else None
    ...
    if args.fast_mode and len(transactions) > 0:
        # Keep a manageable subset for quick runs
        max_txn = min(100_000, len(transactions))
        transactions = transactions[:max_txn]
        labels = labels[:max_txn]
